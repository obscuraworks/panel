...
// [ADDED] Import request baru untuk toggle visibility
use Pterodactyl\Http\Requests\Api\Client\Servers\Settings\ToggleVisibilityRequest;
...
class SettingsController extends ClientApiController
{
    ...
    /**
     * [ADDED] Toggles the hide_from_admin flag for a server.
     *
     * Hanya owner server yang bisa memanggil endpoint ini (dijaga oleh
     * ToggleVisibilityRequest::authorize()). Subuser dan admin lain
     * akan mendapat 403 Forbidden.
     *
     * @throws \Throwable
     */
    public function toggleVisibility(ToggleVisibilityRequest $request, Server $server): JsonResponse
    {
        $hideFromAdmin = (bool) $request->input('hide_from_admin');

        $server->forceFill(['hide_from_admin' => $hideFromAdmin])->saveOrFail();

        Activity::event('server:settings.visibility')
            ->property(['hide_from_admin' => $hideFromAdmin])
            ->log();
