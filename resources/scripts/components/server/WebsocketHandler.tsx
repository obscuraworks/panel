...
export default () => {
    ...
    // Jika admin mengakses server dengan privacy mode aktif, jangan konek websocket sama sekali.
    // Ini mencegah console stuck loading spinner karena token request akan di-403 oleh server.
    const isServerOwner = ServerContext.useStoreState((state) => state.server.data?.serverOwner ?? false);
    const hideFromAdmin = ServerContext.useStoreState((state) => (state.server.data as any)?.hideFromAdmin ?? false);
    const rootAdmin = useStoreState((state) => state.user.data!.rootAdmin);
    const isAdminBlocked = rootAdmin && !isServerOwner && hideFromAdmin;
    ...
    useEffect(() => {
        // Admin yang di-block oleh privacy mode tidak perlu (dan tidak bisa) konek websocket.
        // Token request akan di-403 oleh ServerPolicy, jadi kita skip koneksi sepenuhnya.
        if (isAdminBlocked) {
            return;
        }
    ...
    // Admin yang di-block: jangan render error/spinner apapun — ServerConsoleContainer
    // sudah handle tampilannya dengan AdminPrivacyBlock.
    if (isAdminBlocked) {
        return null;
    }
